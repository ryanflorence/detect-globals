...
function getContext(node) {
  ...
  if (node.type === 'CallExpression' &&
    ...
    return (arg.type === 'ThisExpression' || arg.value == null)? GLOBAL_CONTEXT : arg.value;
  }
...
function addVarToScope(node, declarator) {
  ...
  remove(node.scope, {name: declarator.name});
...
function isGlobal(node, varName){
  ...
  return !declaration ||
    ...
    (declaration.init === 'this' && node.context === GLOBAL_CONTEXT);
}
